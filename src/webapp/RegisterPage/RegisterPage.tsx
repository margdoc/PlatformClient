...
interface RegisterRequest {
  ...
  request: AuthClient.RegisterRequest;
  rpassword: string;
}

interface RegisterResponse {
  type: "RegisterResponse";
  response: AuthClient.RegisterResponse;
}
...
export const reducer: LoopReducer<State, Action> = (state: State, action: Action) => {
  switch (action.type) {
    case "RegisterRequest":
      return action.rpassword === action.request.password
      ? [state, AuthClient.apiRegister<RegisterResponse>(action.request, response => ({
        type: "RegisterResponse",
        response,
      }))
      ]
    ...
    case "RegisterResponse":
      action.response.type === "RegisterErrorResponse"
      ? [{
        ...state,
        errors:
          typeof action.response.response.detail === "string"
          ? [action.response.response.detail]
          : action.response.response.detail.map(error => 
